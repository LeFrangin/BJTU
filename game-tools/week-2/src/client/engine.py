...
class Game( object ):
	...
	def choosing( self, choosing=True ):
		## Mouse position
		mouse = pygame.mouse.get_pos()
		...
		## Draw title
		if ( choosing ):
			self.drawMessage( "Choose your weapon", self.__bag.screen.get_rect().width / 2, self.__bag.screen.get_rect().height / 15, True, 40, "white" )
			...
			try :
				...
				if ( ( mouse[ 0 ] >= x and mouse[ 0 ] < ( x + width )  ) and ( mouse[ 1 ] >= y and mouse[ 1 ] < ( y + height )  ) ):
					...
					if ( choosing and pygame.mouse.get_pressed()[ 0 ] ):
						...
						self.__bag.messageNetwork = { "state": 3, "id": object[ "id" ] }
				...
				x += width
				if ( x > self.__bag.screen.get_rect().width ):
					x = 0
					y += height
